...
def handle_message(event):
    ...
    if message_text == "使用說明":
        ...
        message=Products.list_all()
            ...
            product_name=message_text.split(',')
            num_item=message_text.rsplit(':')[1]
            product=db_session.query(Products).filter(Products.name.ilike(product_name))
            if product:
                ...
                message=TemplateSendMessage(alt_text="Sorry,We don't have {}.".format(product_name))
    ...
    line_bot_api.reply_message(
        event.reply_token, TextSendMessage(text='Hi! Welcome to LSTORE.')).first()
